...
@Entity
@Table(
    name = "preparecomputerequests"
    )
@Inheritance(
    strategy = InheritanceType.JOINED
    )
public class PrepareComputeResourceRequestEntity
extends ComputeResourceProcessingRequestEntity
implements PrepareComputeResourceRequest
    {

    protected PrepareComputeResourceRequestEntity()
        {
        super();
        }

    protected PrepareComputeResourceRequestEntity(final AbstractComputeResourceEntity compute)
        {
        super(
    ...
    public ProcessingAction preProcess(final RequestProcessingPlatform platform)
        ...
        this.getComputeResource().setPhase(IvoaLifecyclePhase.PREPARING);
        //
        // Create an Action to do the work outside the database Transaction.
        return new ProcessingAction()
            {
            private UUID requestUuid = PrepareComputeResourceRequestEntity.this.getUuid();
            @Override
            ...
            private UUID   resourceUuid  = PrepareComputeResourceRequestEntity.this.getComputeResource().getUuid();
            private String resourceClass = PrepareComputeResourceRequestEntity.this.getComputeResource().getClass().getSimpleName();
            ...
            public boolean process()
                ...
                try {
                    ...
                    {
                    log.debug("InterruptedException [{}][{}] while preparing resource [{}][{}]", ouch.getClass().getSimpleName(), ouch.getMessage(), resourceUuid, resourceClass);
                    }
                return true;
                }
            };
    ...
    public void postProcess(final RequestProcessingPlatform platform)
        ...
        //
        // TODO Check that the compute is prepared ....
        //
        //
        // Mark our resource as AVAILABLE.
        this.getComputeResource().setPhase(IvoaLifecyclePhase.AVAILABLE);
        //
        // Activate our parent Session. 
        platform.getSessionProcessingRequestFactory().createSessionAvailableRequest(
            (SimpleExecutionSessionEntity) this.getComputeResource().getSession()
            );
        //
        // Close this request.
