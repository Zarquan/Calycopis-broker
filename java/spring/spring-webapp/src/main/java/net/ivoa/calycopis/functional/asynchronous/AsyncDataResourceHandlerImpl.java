...
implements AsyncDataResourceHandler
    ...
    extends AsyncLifecycleComponentHandlerImpl.InnerLifecycleHandler<AbstractDataResourceEntity>
        {

        @Autowired
        InnerLifecycleHandler(
            final AbstractDataResourceEntityRepository dataRepository
            ){
        ...
        void doPreparing(final UUID uuid, final AtomicInteger counter)
            {
            AbstractDataResourceEntity entity = this.repository.findById(
                uuid
                ).orElseThrow();
